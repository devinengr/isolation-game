...
import onl.devin.isolationgame.util.Heuristic;
import onl.devin.isolationgame.util.HeuristicPairNode;
import onl.devin.isolationgame.util.SecondHeuristicUtil;

import java.util.*;

public class AdversarialH2 {

    private static final int AB_MAX_MOVE_COUNT = 3;
    private static final int AB_MAX_TOKEN_COUNT = 10;
    private static final int ITERATIVE_DEEPENING_LEVELS = 3;

    /**
     * perform adversarial search. wreck the opponent without
     * grace nor honor (probably less caffeine at 3am btw)
    ...
    private void evaluateForEachLevel(MockStateNode parentNode) {
        ...
        while (!nodeQueue.isEmpty()) {
            ...
            List<Heuristic> moves = SecondHeuristicUtil.calculateFullBestMoveHeuristics(gameState);
            ...
            for (Heuristic move : moves) {
                List<Heuristic> tokens = SecondHeuristicUtil.calculateFullBestTokenHeuristics(gameState);

                // randomize the order of the list
                // to avoid bias in the algorithm.
                // prune some options to speed up the
                // algorithm, but risks losing the best move.
